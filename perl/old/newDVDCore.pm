    ...
    foreach (@$function_data) {
        ...
        until ( $fn == $max ) {
            if ( $max > 1 ) {    #function stochastic
                ...
                _log("Reading fn $fn, function: $line");
            }
            ...
            _log(
                "Starting check of function $fcount, which has $max function(s)... f$fcount, $line"
            );
...
# don't count lines with -> in them
    ...
    if ( $statespace == 1 ) {
        ...
        #    {
        if ( -e $dot_filename ) {
            ## debug: print "dot -T$fileformat -o $clientip.out.$fileformat $clientip.out.dot";
            $statespace_filename
